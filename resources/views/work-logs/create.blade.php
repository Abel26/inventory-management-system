<x-app-layout>
    <x-slot name="title">{{ __('work_logs.page.create_title') }}</x-slot>
...
<div class="min-h-screen bg-[#f8fafc] py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
        ...
        <!-- Form Card -->
        <div class="bg-white shadow-2xl shadow-indigo-100 rounded-[2.5rem] border border-slate-100 p-8 sm:p-12 relative overflow-hidden">
            <!-- Decorative Element -->
    ...
    .then(data => {
        if (data.success) {
            Swal.fire({
                ...
                text: data.message || '{{ __('work_logs.messages.created') }}',
                confirmButtonColor: '#009B77',
                timer: 2000,
                timerProgressBar: true,
                showConfirmButton: false
            }).then(() => {
                window.location.href = '{{ route('work-logs.my-work') }}';
            });
        } else {
            let errorMsg = data.message || '{{ __('work_logs.messages.create_failed') }}';
