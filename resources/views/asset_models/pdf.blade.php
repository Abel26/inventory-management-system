...
<head>
    <meta charset="UTF-8">
    <title>{{ $title ?? 'Laporan Model Aset' }} - PT. EBARA INDONESIA</title>
    <style>
        @page {
...
<body>
    ...
    <div class="report-title">
        <h2>{{ $title ?? 'Laporan Data Model' }}</h2>
        <p>{{ $subtitle ?? 'Data Inventaris Model dan Cetakan' }}</p>
    </div>
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 10%;">Tgl Pembuatan</th>
                ...
                <tbody>
                    @foreach($models as $index => $model)
                        <tr>
                            ...
                            <td>{{ $model->name }}</td>
                            <td>{{ $model->type }}</td>
                            <td>{{ $model->material ? $model->material->name : '-' }}</td>
                            <td style="text-align: center;">{{ $model->manufactured_date ? $model->manufactured_date->format('d/m/Y') : '-' }}</td>
                            <td style="text-align: center;">
                                @if($model->condition === 'Good')
                                    <span class="badge badge-success">Baik</span>
                                @elseif($model->condition === 'Repair')
                                    <span class="badge badge-warning">Perbaikan</span>
                                @elseif($model->condition === 'Damaged')
            ...
            <table class="summary-table">
                <tr>
                    <td style="width: 50%; vertical-align: top; padding: 10px; border: 1px solid #ddd;">
                        ...
                        Total Model: <strong>{{ $models->count() }}</strong> item<br>
                        Kondisi Baik: <strong>{{ $models->where('condition', 'Good')->count() }}</strong> item<br>
                        Perlu Perbaikan: <strong>{{ $models->where('condition', 'Repair')->count() }}</strong> item<br>
                        Rusak: <strong>{{ $models->where('condition', 'Damaged')->count() }}</strong> item
                    </td>
                    <td style="width: 50%; vertical-align: top; padding: 10px; border: 1px solid #ddd;">
                        <strong>Distribusi per Tipe:</strong><br><br>
                        @foreach($models->groupBy('type') as $type => $items)
                            {{ $type }}: <strong>{{ $items->count() }}</strong> item<br>
                        @endforeach
                        <br>
